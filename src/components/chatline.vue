    ...
    methods: {
        ...
        notify (msg) {//消息提示
            ...
            if (!("Notification" in window)) {
                alert("你的浏览器不支持桌面消息推送，请下载谷歌浏览器使用该功能");
            }
            // 检查用户是否同意接受通知
                ...
                var notification = new Notification("有新消息",options);
            }
            // 否则我们需要向用户获取权限
        ...
        connect: function() {
            let self = this
            var socket = new SockJS('http://paiwallet.88download.com/chat/chat-webSocket');
            ...
            stompClient.connect({}, function(frame) {
                stompClient.subscribe('/user/' + self.msg.myId + '/' + self.orderId, function(response) {
                    ...
                    self.notify(self.otheritem.content)
        ...
        getBefore: function(page) {
            ...
            this.$http.post('http://paiwallet.88download.com/chat/getHistoryMessage', params).then(response => {
