        ...
        <div class="nav-right col-xs-12 col-md-10 padding-right-clear">
            <div class="bill_box rightarea padding-right-clear record">
                <section class="trade-group merchant-top">
                    ...
                    <span class="tips-word">提现</span>
                    <span class="head-group">
                        <router-link to="/withdrawAddr">提现地址管理</router-link>
                    </span>
                </section>
                <section>
                    <div class="table-inner action-box">
                        ...
                        <div class="action-inner">
                            ...
                            <div class="inner-box">
                                <div class="form-group form-address">
                                    <label for="controlAddress" class="controlAddress describe">提现地址</label>
                            ...
                            <div class="form-group form-amount">
                                <label class="label-amount"> 数量
                                    <p class="label-fr">
                                        ...
                                        <span v-if="currentCoin.enableAutoWithdraw == 0">【温馨提示】：提现数量低于 {{currentCoin.threshold}} 时自动到账，否则需要人工审核</span>
                                        <span>
                                ...
                                <div class="input-group" style="margin-right:-50px">
                                    <Poptip trigger="focus" :content="'最多输入6位小数,最小值为'+currentCoin.minAmount+' ,最大值为'+currentCoin.maxAmount" style="width: 100%;">
                                        <InputNumber @on-change="computerAmount" v-model="withdrawAmount" placeholder="输入提现数量" size="large" style="width: 434px;"></InputNumber>
                        ...
                        <div class="form-group-container form-group-container2">
                            <div class="form-group form-fee">
                                ...
                                <div class="input-group">
                                    <Slider v-model="withdrawFee"  @on-change="computerAmount2" show-input :step="(currentCoin.maxTxFee - currentCoin.minTxFee)/10" :max="currentCoin.maxTxFee" :min="currentCoin.minTxFee"></Slider>
                                    <!--<Poptip trigger="focus" :content="'最多输入6位小数,最小值为'+currentCoin.minTxFee+'最大值为'+currentCoin.maxTxFee" style="width: 100%;">
                                        <InputNumber @on-change="computerAmount2" v-model="withdrawFee" placeholder="输入提现费率" :min="currentCoin.minTxFee" :max="currentCoin.maxTxFee" size="large" style="width: 440px;"></InputNumber>
                        ...
                        <div class="action-foot">
                            <Button id="withdrawSubmit" long size="large" type="primary" style="height:40px;" @click="apply">提现</Button>
                        </div>
                        <div class="action-content pt10">
                            <div class="action-body">
                                <p class="acb-p1">温馨提示</p>
                                <p class="acb-p2">• 最小提现数量为：{{currentCoin.minAmount}}币。<br>• 为保障资金安全，当您账户安全策略变更、密码修改、使用新地址提现，我们会对提现进行人工审核，请耐心等待工作人员电话或邮件联系<br>• 请务必确认电脑及浏览器安全，防止信息被篡改或泄露。 </p>
                            </div>
                        </div>
                        <div class="action-content">
                            <div class="action-body">
                                <p class="acb-p1">提现记录</p>
...
export default {
    ...
    methods: {
        ...
        accSub(arg1, arg2) {//减法：javascript的减法结果会有误差，在两个浮点数相加的时候会比较明显。这个函数返回较为精确的减法结果。
            var r1, r2, m, n;
        ...
        valid() {
            if (this.coinType == '') {
                ...
                this.$Message.error('请填写正确提现数量，最小值为' + this.minAmount);
                return false
            } else if (this.desc == '') {
                this.$Message.error('请填写备注');
                return false
            } else if ((this.withdrawAmount - 0) < this.withdrawFee) {
                this.$Message.error('提现数量不得小于手续费');
                return false
        ...
        ok6() {
            ...
            this.$http.post(this.host + '/uc/withdraw/apply', params).then(response => {
                ...
                if (resp.code == 0) {
                    this.$Modal.success({
                        title: '',
                        content: '<h1 style="margin-left:80px">提现成功!</h1>'
                    });
