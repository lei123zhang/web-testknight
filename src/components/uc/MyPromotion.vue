        ...
        <div class="nav-right col-xs-12 col-md-10 padding-right-clear">
            <div class="bill_box rightarea padding-right-clear record account-box">
                ...
                <div class="message" v-else-if="this.changeActive==1">
                    <Table stripe :columns="tablePromoteFriends" :data="dataPromoteFriend" :loading="loading"></Table>
                </div>
                <div class="message" v-else>
                    <Table stripe :columns="tablePromoteMoneylist" :data="dataPromoteMoney" :loading="loading"></Table>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        qrcodeM(){
            this.qrcode.value="http://www.fexchain.io/#register?agent="+this.member.promotionCode;
        },
        onCopy(e) {
        ...
        onError(e) {
         ...
         getPromotionList(){
          this.$http.post(this.host + '/uc/promotion/record', {}).then(response => {
            ...
            if (resp.code == 0) {
              this.dataPromoteFriend = resp.data;
            } else {
