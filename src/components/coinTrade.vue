<template>
    <div class="nav-right tradeCenter" style="height:1200px">
        <Row  type="flex" justify="start" class="code-row-bg" style="background:#fff">
            <Col span='10'>
            &nbsp;
                 <!-- <section class="trade-group merchant-top">
                    <Checkbox label="" v-model="isCertified" style="margin-left:60px;" @on-change="showBs">
                        <span style="font-size:16px;color:#7a7a7a;">仅显示</span>
                        <span style="color:#b3d5fa;font-size:16px;">  认证商家？</span>
                    </Checkbox>          
               </section> -->
            </Col>
             <Col span='4' offset="10" ><router-link to="/editad" class="sendad">发布广告</router-link></Col>
        </Row>
...
export default {
    ...
    methods: {
        ...
        if(this.isCertified==true){
              //  alert('true')
                this.isCertifiedCode=1
                 this.getAd(1,1,0);
            }else{
                this.isCertifiedCode=0
                 this.getAd(1,1,0);
            // alert('false')
            };
        },
        getAd(pageNo,advertiseT) {
            //获取广告
            let params = {
                pageNo:pageNo,
                pageSize:10,
                advertiseType:advertiseT,
                unit: this.coin,
                isCertified:this.isCertifiedCode
            };
...
<style >
.sendad{color:#fff;line-height:84px;font-size:16px;padding:8px;background:#b03416;border-radius:5px}
.payMode-margin{
  margin-right:8px;
}
