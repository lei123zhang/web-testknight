<template>
    <div class="content-wrap">
                    ...
                    <div class="rightbox">
                        ...
                        <Row class="chat-top" type="flex" justify="space-between" v-show="statusBtn!=0">
                            ...
                            <Col span="8" class="order-info" v-if="alipay&&alipay!=null">
                                ...
                                <pre></pre>
                                <p>{{alipay.aliNo}}</p>
                                <p v-if="alipay&&alipay!=null&&alipay.qrCodeUrl!=null&&alipay.qrCodeUrl!=''"><a @click="showQRCode(1)">二维码</a></p>
                            </Col>
                            <Col span="8" class="order-info" v-else>
                                <i class="icons alipay"></i>
                                <pre></pre>
                            ...
                            <Col span="8" class="order-info" v-if="wechatPay&&wechatPay!=null">
                                ...
                                <p v-if="wechatPay&&wechatPay!=null&&wechatPay.qrWeCodeUrl!=null&&wechatPay.qrWeCodeUrl!=''"><a @click="showQRCode(2)">二维码</a></p>
                            </Col>
                            <Col span="8" class="order-info" v-else>
                                <i class="icons wechat"></i>
                                <pre></pre>
        ...
        <Modal v-model="modal6" title="提示" @on-ok="ok6('formValidate')">
            <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="85">
                ...
                <!-- <FormItem>
                    <Button type="primary" @click="ok6('formValidate')">保 存</Button>
                </FormItem> -->
            </Form>
        </Modal>
        <!-- 二维码 -->
...
export default {
    ...
    methods: {
        ...
        watchOrderStutusNotice: function() {
            ...
            this.stompClient.connect({}, function(frame) {
                ...
                function(response) {
                    ...
                    self.statusText = '等待付款';
                     self.setReserveTime();
                    self.setReserveTime();
                    } else if (confirmPayMsg.status == 2) {
                    self.statusText = '等待放行';
        ...
        getReserveTime:function () {
            ...
            var reserveSeconds = parseInt(this.msg.timeLimit)*60 - throughSeconds-50;
        ...
        ok6(name) {
            this.$refs[name].validate((valid) => {
                if (valid) {
                    this.$http.post(this.host + '/otc/order/release', { orderSn: this.$route.query.tradeId,jyPassword:this.formValidate.password }).then(response => {
                        var resp = response.body;
                        if (resp.code == 0) {
